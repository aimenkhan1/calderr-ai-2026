...
## 📝 Portfolio Checklist (from the project spec)

- [x] GitHub repo structure with full system design README (this file)
- [x] Docker Compose deployment — starts with one command
- [x] FastAPI API with OpenAPI docs
- [x] Streamlit UI showing phase-by-phase agent progress
- [x] Dynamic agent selection verified across all 3+ domains (tested across all 5)
- [x] All 5 research phases execute and produce typed outputs (verified via automated tests)
- [x] Critic Agent modifies findings — verified in testing (see below)
- [ ] **5 complete research reports on different domains** — mocked/schema-verified versions included; **regenerate with a real API key before submitting** (see note above)
- [x] Architecture diagram — `architecture_diagram.mmd` (auto-generated from the live graph; paste into [mermaid.live](https://mermaid.live) to export PNG/SVG)
- [ ] **Recorded 5-minute demo video** — not generated by this build; record a walkthrough of `docker compose up` → Streamlit run → FastAPI `/docs` once you've regenerated real sample reports
- [x] **Blog post draft** — `BLOG_POST_DRAFT.md` is ready to edit and publish to dev.to or Hashnode; update the GitHub link at the bottom before publishing, then link it from this README
...
**What you still need to do for a fully authentic submission:** run the
real pipeline with your own `GROQ_API_KEY` to generate genuine sample
reports and confirm the "Critic modifies at least one finding in 3 of 5
runs" evaluation criterion against real model behavior — the mocked tests
above prove the *mechanism* works, not that a live model will always
trigger it at that exact rate.
